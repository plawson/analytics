...
public class App
{
    public static void main( String[] args ) throws AlreadyAliveException, InvalidTopologyException, AuthorizationException
    {
        ...
        builder.setBolt("user-page-visit-counts", new UserPageVisitCount()
                .withWindow(BaseWindowedBolt.Duration.of(1000*60*60), BaseWindowedBolt.Duration.of(1000*30)), 2)
                .fieldsGrouping("page-visits", new Fields("url"));
        StormTopology topology = builder.createTopology();

        Config config = new Config();
        config.setMessageTimeoutSecs((60*60) + 40); // Message timeout > window length + window sliding duration
